...
/**
 * GameView class takes care of making our scene for the board simulation and also the
 * starting scene before we launch the actual simulation. It also has the responsibility
 * of reading the css files used for different layouts of or simulation window. In addition, it is
 * also where we create alert widows and messages to pop-up when the code run into exceptions.
 *
 * @author - Justin, Grace, Luisa
 */
